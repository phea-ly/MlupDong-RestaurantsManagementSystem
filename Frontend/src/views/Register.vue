...
import { registerSession, getDashboardPathByRole } from '@/utils/auth'
...
async function register() {
  ...
  loading.value = true
  try {
    await registerSession({ name: name.value, email: email.value, password: password.value })
    router.push(getDashboardPathByRole())
  } catch (e) {
  ...
  <v-app>
    <v-main class="login-bg">
      <v-container class="d-flex align-center justify-center" style="min-height: 100vh">
        <v-card width="420" rounded="xl" elevation="0" class="pa-6 login-card">
          ...
          <p class="text-center mt-4" style="font-size:13px; color:#7a899f">
            ...
            <router-link to="/login" style="color:#0f9e5f; font-weight:700">Sign In</router-link>
          </p>
