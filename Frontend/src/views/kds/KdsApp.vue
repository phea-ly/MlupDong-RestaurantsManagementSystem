...
// Columns config keeps the template DRY
const columns = computed(() => [
  { status: "incoming",  label: "Incoming",  orders: incomingOrders.value  },
...
<style scoped>
:global(:root) { --app-font: 'Plus Jakarta Sans', 'Segoe UI', system-ui, sans-serif; }
:global(body)  { background: #f6f7fb; font-family: var(--app-font); }
:global(.v-application) { font-family: var(--app-font); }

.kds-app  { background: #f6f7fb; min-height: 100vh; color: #1f2937; }
.kds-bar  { background: #ffffff; border-bottom: 1px solid #e5e7eb; padding: 0 12px; }

.kds-brand        { display: flex; align-items: center; gap: 10px; }
.kds-brand__title { font-size: 16px; font-weight: 700; }
.kds-clock        { font-size: 12px; color: #6b7280; font-family: monospace; margin-left: 6px; }
...
.kds-badge :deep(.v-badge__badge) { font-size: 10px; font-weight: 700; }
.kds-live    { margin-left: 12px; margin-right: 6px; }
.kds-refresh { margin-left: 4px; }

.kds-body  { padding: 20px; }
.kds-board { margin-top: 4px; }
...
.kds-col__dot   { width: 8px; height: 8px; border-radius: 50%; }
