<template>
  <div class="cart-layout pb-24">
    <!-- Header -->
    <div
      class="d-flex flex-column align-center bg-white pt-5 pb-4 position-sticky top-0 sticky-header z-10 elevation-0">
      <v-btn icon="mdi-arrow-left" variant="text" size="small" @click="router.back()" class="position-absolute"
        style="left: 12px; top: 18px;" color="grey-darken-3"></v-btn>
      <div class="text-h6 font-weight-black line-height-1 text-grey-darken-4 mb-1">Your Cart</div>
      <div class="text-caption font-weight-bold text-green-darken-3 text-uppercase letter-spacing-1"
        style="font-size: 10px;">Mlup Dong • Table 05</div>
    </div>
    ...
    <div v-if="cartStore.items.length === 0" class="empty-state text-center px-6 py-10 mt-10">
      ...
      <v-btn color="#215732" rounded="xl" elevation="2" @click="router.push('/customer-menu')"
        class="px-8 font-weight-bold">Browse Menu</v-btn>
    </div>

    <div v-else class="px-4 pt-6 pb-6">
      <!-- Cart Items -->
      <v-card v-for="item in cartStore.items" :key="item.id"
        class="cart-item-card rounded-xl pa-3 mb-4 d-flex align-stretch">
        <v-img :src="item.image || fallbackImg" width="100" height="100"
        ...
        <div class="flex-grow-1 d-flex flex-column justify-space-between pl-1 py-1">
          <div class="d-flex justify-space-between align-start">
            <div class="font-weight-bold text-grey-darken-4 line-clamp-2 pr-2"
          ...
          <div class="d-flex justify-space-between align-end mt-2 pb-1">
            ...
            <div class="quantity-control d-flex align-center rounded-pill pa-1 pl-2 pr-2">
              <v-icon size="18" :color="item.quantity <= 1 ? 'grey-lighten-1' : 'grey-darken-3'"
                @click="cartStore.updateQuantity(item.id, item.quantity - 1)" class="cursor-pointer">mdi-minus</v-icon>
              <div class="px-4 font-weight-black text-body-2">{{ item.quantity }}</div>
              <v-btn icon size="24" color="#215732" class="rounded-circle elevation-0"
      ...
      <div class="mt-6 mb-8">
        ...
        <v-textarea v-model="cartStore.specialInstructions"
          ...
          bg-color="white" rounded="xl" hide-details auto-grow rows="3" color="#215732"
    ...
    <v-slide-y-reverse-transition>
      <div v-if="cartStore.items.length > 0"
        class="fixed-bottom-checkout bg-white pt-5 pb-6 px-5 elevation-10 rounded-t-xl">
        ...
        <v-btn color="#215732" rounded="xl" block size="x-large"
          class="font-weight-black text-white elevation-2 place-order-btn" @click="placeOrder"
          :loading="isPlacingOrder">
          <span class="mr-2">PLACE ORDER</span>
          <v-icon size="20">mdi-rocket-launch</v-icon>
        </v-btn>
        <div class="text-center mt-3 text-grey-lighten-1"
