<template>
  <div class="cart-layout pb-24">
    ...
    <div v-if="cartStore.items.length === 0" class="empty-state text-center px-6 py-10 mt-10">
      ...
      <v-btn color="#215732" rounded="xl" elevation="2" @click="cartStore.tableToken ? router.push(`/menu/${cartStore.tableToken}`) : router.back()" class="px-8 font-weight-bold">Browse Menu</v-btn>
    </div>
...
async function placeOrder() {
  ...
  const result = await cartStore.placeOrder({
    order_type: 'dine_in',
    table_id: 1 // In a real app, this would come from a QR code or session
  })
  ...
  if (result.success) {
    ...
    if (cartStore.tableToken) {
      router.replace(`/menu/${cartStore.tableToken}`)
    } else {
      router.back()
    }
  } else {
