...
<template>
  <div class="cart-layout pb-24 fade-in">
    ...
    <div class="d-flex align-center bg-white pt-4 pb-3 px-4 sticky-header justify-space-between">
      <v-btn icon="mdi-chevron-left" variant="flat" size="small" color="#f3f5f7" class="border-subtle"
        @click="$router.back()" />
    ...
    <div v-if="cartStore.items.length === 0 && !showSuccess"
      class="empty-state d-flex flex-column align-center justify-center px-10">
    ...
    <div v-else-if="!showSuccess" class="px-4 pt-6">
      ...
      <v-card v-for="item in cartStore.items" :key="item.id"
        ...
        <v-img :src="item.image || fallbackImg" width="88" height="88"
        ...
        <div class="flex-grow-1 d-flex flex-column justify-center py-1">
          <div class="d-flex justify-space-between align-start mb-1">
            <span class="font-weight-black text-grey-darken-4" style="font-size:15px;line-height:1.2">{{ item.name
              }}</span>
          ...
          <div class="d-flex justify-space-between align-center mt-auto">
            ...
            <div class="qty-pill d-flex align-center rounded-pill px-1 border-subtle py-1 bg-slate-50">
              <v-btn icon size="24" variant="flat" color="white" class="border-subtle" :disabled="item.quantity <= 1"
      ...
      <v-card class="rounded-2xl pa-5 mb-8 border-subtle mt-6 bg-white" elevation="0">
        ...
        <v-textarea v-model="cartStore.specialInstructions"
          placeholder="e.g. No onions, extra spicy, or allergy details..." variant="filled" bg-color="#f8fafb"
          rounded="xl" hide-details auto-grow rows="3" color="#215732" flat />
      </v-card>
    </div>
    ...
    <v-slide-y-reverse-transition>
      <div v-if="cartStore.items.length > 0 && !showSuccess" class="checkout-panel rounded-t-3xl pt-7 pb-9 px-6">
        ...
        <v-btn color="#215732" rounded="xl" block size="x-large" class="font-weight-black elevation-8 checkout-btn py-6"
    ...
    <v-overlay v-model="showSuccess" class="align-center justify-center" scrim="#133021" persistent>
      <v-card class="pa-0 text-center rounded-3xl overflow-hidden glass-success" width="340">
        <div class="pa-10 d-flex flex-column align-center">
          ...
          <v-btn color="white" block rounded="xl" size="large" class="text-green-darken-4 font-weight-bold"
            @click="goBackToMenu">
            Back to Menu
          </v-btn>
        </div>
...
.wait-time-card {
  border: 1px solid #c6e9d3 !important;
}
