...
const displayTableNumber = computed(() => {
  ...
  if (num.length > 8 && num !== '??') return `${num.substring(0, 4)}...${num.slice(-4)}`
  return num
})
...
onMounted(async () => {
  ...
  // If tableId already set (navigating from menu), skip token resolve
...
async function placeOrder() {
  ...
  // Set showSuccess BEFORE awaiting — prevents the watch from
  // redirecting to menu when clearCart() empties the cart mid-await.
...
// ── Watches ────────────────────────────────────────────────────────────────
// If cart empties (e.g. user clears all) and success isn't shown, go back
watch(
  ...
  <div class="cart-layout">
    ...
    <template v-else>
      ...
      <div class="sticky-header d-flex align-center px-4 py-3">
        ...
        <!-- Item count badge -->
        <v-chip v-if="cartStore.cartCount > 0" size="small" color="#2f6b3c" variant="tonal"
          class="font-weight-black">
          {{ cartStore.cartCount }} item{{ cartStore.cartCount !== 1 ? 's' : '' }}
        </v-chip>
      </div>
      ...
      <div v-if="cartStore.items.length === 0 && !showSuccess"
        ...
        <div class="empty-blob mb-8 d-flex align-center justify-center"
          style="width:120px;height:120px;background:rgba(47,107,60,.08);border-radius:50%">
        ...
        <v-btn class="brand-btn text-white" rounded="pill" elevation="0"
          @click="goBackToMenu" height="54" width="100%" style="max-width:280px">
      ...
      <div v-else-if="!showSuccess" class="px-4 pt-4 pb-cart fade-in">
        ...
        >
          ...
          <div class="cart-item-info">
            ...
            <div class="cart-item-desc mb-3">Fresh ingredients – Best quality</div>
        ...
        <v-textarea
          ...
          hide-details auto-grow rows="3" color="#2f6b3c"
        ...
        <div class="wait-card d-flex align-center ga-3 mb-2">
          ...
          <div>
            <div class="text-caption font-weight-bold" style="color:#5a6e57">Estimated wait</div>
        ...
        <v-alert
          ...
          closable density="compact" class="mb-3 text-body-2 font-weight-bold"
      ...
      <v-slide-y-reverse-transition>
        <div v-if="cartStore.items.length > 0 && !showSuccess" class="checkout-panel px-6 pt-5 pb-6">
          ...
          <div class="text-center text-uppercase"
            style="font-size:9px;color:#9aab96;letter-spacing:.5px;font-weight:600">
      ...
      >
        <v-card class="pa-0 text-center overflow-hidden bg-white success-card" elevation="16">
          <div class="pa-8 d-flex flex-column align-center">
            ...
            <h2 class="header-title mb-2" style="font-size:26px">Order Sent!</h2>
            ...
            <div class="wait-badge mb-5">
              <div class="text-caption font-weight-black text-uppercase"
                style="opacity:.6;font-size:10px;color:#1c2e1a;letter-spacing:.08em">
                Estimated Wait
              </div>
              <div class="price-text" style="font-size:40px;line-height:1.1">
                ~{{ confirmedWaitMinutes }}<span style="font-size:20px">m</span>
              </div>
            </div>
            ...
            <div v-if="cartStore.lastOrder"
              class="text-caption mb-5" style="color:#9aab96">
...
.brand-btn {
  ...
  box-shadow: 0 4px 16px rgba(47,107,60,.25) !important;
}
...
.sticky-header {
  ...
  box-shadow: 0 4px 24px rgba(30,50,25,.04);
}

/* ── Empty state ────────────────────────────────────────────────────────── */
.empty-state { height: calc(100vh - 64px); }
...
.cart-item-card {
  ...
  box-shadow: 0 4px 18px rgba(30,50,25,.05);
...
.cart-item-img { width: 100%; height: 100%; }
.cart-item-img-shine {
...
.cart-item-desc {
  font-size: 11px; color: #7a8c76; line-height: 1.3;
  display: -webkit-box;
  -webkit-line-clamp: 1;
  -webkit-box-orient: vertical;
  overflow: hidden;
...
.cart-item-price {
  font-family: 'Playfair Display', serif;
  font-size: 18px; font-weight: 700; color: #2f6b3c;
  display: flex; align-items: baseline; gap: 2px;
}
.currency-symbol {
  font-family: 'DM Sans', sans-serif;
  font-size: 13px; color: #5a6e57; font-weight: 600;
}
...
.qty-val {
  ...
  color: #1c2e1a; min-width: 14px; text-align: center;
}

/* ── Chef note textarea ─────────────────────────────────────────────────── */
.instruction-textarea :deep(.v-field__outline) { --v-field-border-opacity: 0.1 !important; }
...
.instruction-textarea :deep(.v-field__outline::after) { display: none !important; }
...
.success-icon-box {
  ...
  border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
  border: 1px solid rgba(47,107,60,.12);
}
.wait-badge {
  background: #f0f7f2;
  border-radius: 16px;
...
.countdown-badge.active {
  background: rgba(47,107,60,.14);
  color: #1a4526;
  border-color: rgba(47,107,60,.3);
}
...
.dot.active {
  background: #2f6b3c;
  box-shadow: 0 0 8px rgba(47,107,60,.4);
}
