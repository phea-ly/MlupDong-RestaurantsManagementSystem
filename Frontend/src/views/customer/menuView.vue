  ...
  onMounted(async () => {
    ...
    if (tableToken.value) {
      cartStore.setTableToken(tableToken.value)
      try {
  ...
  <template>
    <div class="mobile-layout">
      ...
      <div class="main-content-bg mt-3 rounded-t-xl pt-4">
          ...
          <v-card v-for="i in 6" :key="i" class="mb-4 rounded-xl elevation-1 bg-white">
            ...
            <div class="pa-3">
              ...
              <div class="d-flex align-center justify-space-between">
                ...
                <v-btn icon color="#215732" size="44" class="text-white elevation-2" disabled>
        ...
        <!-- Featured Item -->
        <div v-if="featuredItem" class="px-4 mb-6 mt-4 fade-in">
          <h2 class="text-h6 font-weight-black mb-3 text-grey-darken-4">Our Speciality</h2>
          <v-card class="featured-card rounded-xl overflow-hidden elevation-2 bg-white" v-ripple>
            <div class="position-relative">
              <v-img :src="featuredItem.image || fallbackImg" height="200" cover class="align-end">
                <div class="gradient-overlay"></div>
              </v-img>
              <div class="price-badge font-weight-black shadow-sm">${{ parseFloat(featuredItem.price).toFixed(2) }}</div>
            </div>
            <div class="pa-4">
              <div class="d-flex align-center justify-space-between mb-1">
                <div class="text-h6 font-weight-black text-grey-darken-4 line-height-1">{{ featuredItem.name }}</div>
                <v-btn icon color="#215732" size="44" class="text-white elevation-2 btn-add-animated"
                  @click="addToCart(featuredItem, $event)">
                  <v-icon size="24">mdi-plus</v-icon>
                </v-btn>
              </div>
              <div class="text-body-2 text-grey-darken-1 mb-2">{{ featuredItem.description || 'Our chef\'s special recommendation for you.' }}</div>
            </div>
          </v-card>
        </div>
        ...
        <!-- Empty State -->
        <div v-if="!menuStore.loading && !filteredItems.length" class="px-8 py-16 text-center fade-in">
          <v-icon size="80" color="grey-lighten-2" class="mb-4">mdi-silverware-clean</v-icon>
          <div class="text-h6 font-weight-bold text-grey-darken-3 mb-1">Menu is currently empty</div>
          <div class="text-caption text-grey-darken-1">Please check back later or try a different category.</div>
          <v-btn v-if="activeCategory !== 'All' || searchQuery" variant="text" color="#215732" class="mt-4 text-none font-weight-bold" @click="activeCategory = 'All'; searchQuery = ''">
            Clear Filters
          </v-btn>
        </div>
      </div>
