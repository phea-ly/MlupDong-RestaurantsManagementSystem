  ...
  <v-card rounded="xl" border flat class="mb-4">
    <v-card-text class="pa-4">
      ...
      <v-row density="comfortable">
        <v-col cols="12" md="2">
          <v-text-field v-model="filters.date_from" label="Date From" type="date" variant="outlined" density="compact"
            rounded="lg" hide-details />
        </v-col>
        <v-col cols="12" md="2">
          <v-text-field v-model="filters.date_to" label="Date To" type="date" variant="outlined" density="compact"
            rounded="lg" hide-details />
        </v-col>
        <v-col cols="12" md="2">
          <v-select v-model="filters.event_type" :items="['All Events', ...eventTypes]" label="Event Type"
            variant="outlined" density="compact" rounded="lg" hide-details clearable />
        </v-col>
        <v-col cols="12" md="2">
          <v-select v-model="filters.action" :items="['All Actions', ...actions]" label="Action" variant="outlined"
      ...
      <v-row density="comfortable" class="mt-2">
        <v-col cols="12" md="8">
          <v-text-field v-model="filters.search" label="Search" placeholder="Search logs for anything..."
            variant="outlined" density="compact" rounded="lg" hide-details clearable @keyup.enter="$emit('apply')">
        ...
        <v-col cols="12" md="4" class="d-flex align-center justify-end ga-2">
          <v-btn color="var(--app-primary)" rounded="lg" elevation="0" size="small" :loading="loading"
            @click="$emit('apply')">
