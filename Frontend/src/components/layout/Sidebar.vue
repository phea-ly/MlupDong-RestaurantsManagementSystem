...
const profileName = computed(() => auth.user?.first_name + ' ' + auth.user?.last_name || "Sophal K.");
const profileRole = computed(() => {
  const role = auth.user?.role_id ? 'Admin' : 'Manager';
  return role;
});
const profileInitials = computed(() => {
  const first = auth.user?.first_name?.[0] ?? 'S';
  const last  = auth.user?.last_name?.[0]  ?? 'K';
