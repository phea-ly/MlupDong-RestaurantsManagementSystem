...
export const useStaffStore = defineStore("staff", () => {
  ...
  const kitchenCount = computed(
    () => staffList.value.filter((s) => s.role === "CHEF").length,
  );
  const serviceCount = computed(
    () =>
      staffList.value.filter((s) =>
        ["SERVER", "HOST", "WAITRESS", "CASHIER", "STAFF"].includes(s.role),
      ).length,
  );
  ...
  function mapUserOption(u) {
    ...
    const role = u.role_name ?? u.role?.name ?? u.role?.role_name ?? "";
    return {
      id: u.user_id ?? u.id,
      name,
      email: u.email ?? "",
  ...
  function mapStaff(s) {
    ...
    // role_name is a flat field from your formatUser API response
    const role = (u.role_name ?? u.role?.name ?? u.role?.role_name ?? "STAFF")
      .toString()
      .toUpperCase();
  ...
  async function fetchUsers() {
    try {
      ...
      // Get user IDs already assigned as staff
      const staffUserIds = new Set(staffData.map((s) => s.user_id));

      // Excluded roles — never show in staff dropdown
      const excludedRoles = ["admin", "administrator"];

      // Show users who:
      // 1. Are not already a staff member
      // 2. Are not admin
      const available = allUsers.filter((u) => {
        const role = (u.role_name ?? "").toLowerCase();
        return !staffUserIds.has(u.user_id) && !excludedRoles.includes(role);
      });

      userOptions.value = available.map(mapUserOption);
    } catch (err) {
