...
export const useUserStore = defineStore("user", () => {
  ...
  const filterRoleOptions = computed(() => [
    "All Roles",
    ...roles.value.map((r) => r.name ?? r.role_name),
  ]);
  const filterRestaurantOptions = computed(() => [
    "All Locations",
    ...restaurants.value.map((r) => r.name ?? r.restaurant_name),
  ]);
  ...
  function mapUser(u, index = 0) {
    ...
    // ← fix: read role_name flat field first
    const roleName =
      u.role_name ?? u.role?.name ?? u.role?.role_name ?? u.role ?? "";
    const restaurantName =
      u.restaurant?.name ?? u.restaurant?.restaurant_name ?? u.restaurant ?? "";
  ...
  const filteredUsers = computed(() =>
    users.value.filter((u) => {
      const matchRest =
        ["All Locations", "all"].includes(filterRestaurant.value) ||
        u.restaurant === filterRestaurant.value;
      const matchRole =
        ["All Roles", "all"].includes(filterRole.value) ||
  ...
  async function fetchOptions() {
    try {
      ...
      roles.value = Array.isArray(rolesRes.data)
        ? rolesRes.data
        : (rolesRes.data?.data ?? []);
      restaurants.value = Array.isArray(restRes.data)
        ? restRes.data
        : (restRes.data?.data ?? []);
    } catch {
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const idx = users.value.findIndex(
        (u) => u.rawId === editTarget.value.rawId,
      );
      const { data } = await putApi(
        `/users/${editTarget.value.rawId}`,
        payload,
      );
  ...
  async function handleProfileSaved(updatedUser) {
    ...
    try {
      ...
      const { data } = await postApi(
        `/users/${profileTarget.value.rawId}`,
        form,
      );
      const idx = users.value.findIndex(
        (u) => u.rawId === profileTarget.value.rawId,
      );
